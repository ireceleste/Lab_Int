...
isto_temps istogramma_tc(double t[], int nbins, int len, int numero_figura, char ref_istogramma[30], double numero_acquisizione, char nome_figura[30]){
    /*
    Parameters
    ----------
    t : array dei tempi
...
/*   Parameters
    ...
    */ 
    ...
    graphic->SetMarkerStyle(20);
    char v[300]="Istogramma";
    char * title;
    title=strcpy(v,ref_istogramma);
    ...
    graphic->Fit("gaus"); //NON RIESCO A TROVARE poisson
...
void analisi_delta_t(double t[],int len, int numero_figura, char ref_istogramma[30], char nome_figura[30],double numero_acquisizione){
    double Delta_t[len];
    Delta_t[0]=t[1]-t[0];
    ...
    double mean_exp=tot/len;
    double tot1=0;
    for(int i=0; i<len;i++){
        tot1+=pow((Delta_t[i]-mean_exp),2)/len;
    }
    double std_exp=pow(tot1,0.5);
    double minimo=min(Delta_t,len);
    double massimo=max(Delta_t,len);
    ...
    double medie_bins[len];
    double occorrenze_rate[len];
    double x[len];
    double y[len];
    int len_x=0;
    int index[len];
    ...
    graphic->SetTitle(title);
    // Define a function 
    TF1 *f = new TF1("myfunc", "[0]*exp(-x*[1])" , min(x,len_x),max(x,len_x));

    graphic->GetHistogram()->GetYaxis()->SetTitle("Occorrenze in bin di medie_bins[4]-medie_bins[3] s");
...
void analisi_dt_12(double Delta_t12[],int len,int numero_figura,char ref_istogramma[],char nome_figura[30], double numero_acquisizione){
    //////////////////////////////////////////////////
    printf("\n------Stima di %s-------",ref_istogramma);
    double tot=0;
    for(int i=0; i<len;i++){
    ...
    double tot1=0;
    for(int i=0; i<len;i++){
        tot1+=pow((Delta_t12[i]-mean_exp),2)/len;
    }

    double std_exp=pow(tot1,0.5);
    double minimo=min(Delta_t12,len);
    double massimo=max(Delta_t12,len);
    printf("Media dei Delta_t = %lf pm %lf ", mean_exp,std_exp);
    TH1F *histogram1= new TH1F("histogram1","Istogramma1",len, minimo ,massimo+1); //100
    for(int i=0; i<len;i++){ 
        histogram1->Fill(Delta_t12[i]);
    }
    new TCanvas;
    histogram1->Draw();

    
    double medie_bins1[len];
    double occorrenze_rate1[len];
    double x1[len];
    double y1[len];
    int len_x1=0;
    int index1[len];
    for(int i=0;i<len;i++){ //len
        ...
        occorrenze_rate1[i]=histogram1->GetBinContent(i+1);
        medie_bins1[i]=histogram1->GetBinCenter(i+1);
        if(occorrenze_rate1[i]>0){
            x1[len_x1]=medie_bins1[i];
            index1[len_x1]=i;
            len_x1++;
    ...
    double sigma_y1[len_x1];
    double sum_y1=0;//occorrenze totali

    for(int j=0; j<len_x1;j++){
        y1[j]=occorrenze_rate1[index1[j]];
        sigma_y1[j]=pow(y1[j], 0.5); //errore su y: assumo conteggi poissoniani
        sum_y1+=y1[j];
        
    }
    
    new TCanvas;
    TGraphErrors *graphic1 = new TGraphErrors(len_x1,x1,y1,nullptr,sigma_y1);
    
    graphic1->SetMarkerStyle(20);
...
void ciao(){
    ...
    int i;
    int len_t=0;
    FILE * fin=fopen("/home/sarinaleggy/Documenti/lab/Lab_Int/Sciami/Dati/sciami_291122_prova4.dat","r");
    for(i=0; i<N; i++) {
   ...
   */
    ...
    analisi_delta_t(t1,len_t1, n_acquis+6, (char*)"#Delta t telescopio 1", (char*)"ist_dtempi_tel1",durata_acquisizione);

    analisi_delta_t(t2,len_t2, n_acquis+7, (char*)"#Delta t telescopio 2",  (char*)"ist_dtempi_tel2", durata_acquisizione);

    analisi_dt_12(Delta_t12,len_t12, n_acquis+8,(char*) "#Delta t telescopi", (char*)"ist_dtempi_tel12",durata_acquisizione);
